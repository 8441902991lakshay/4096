...
class bootGame extends Phaser.Scene{
    ...
    preload(){
        ...
        this.load.audio("grow", "assets/sounds/grow.ogg");
...
class playGame extends Phaser.Scene{
    ...
    addTile() {
    	let emptyTiles = [];
    	for(let i = 0; i < gameOptions.boardSize.rows; i++) {
    		for(let j = 0; j < gameOptions.boardSize.rows; j++) {
    	...
    	if (emptyTiles.length > 0) {
    		let choosenTile = Phaser.Utils.Array.GetRandom(emptyTiles);
    		this.boardArray[choosenTile.row][choosenTile.col].tileValue = 1;
    		this.boardArray[choosenTile.row][choosenTile.col].tileSprite.visible = true;
    		this.boardArray[choosenTile.row][choosenTile.col].tileSprite.setFrame(1);
    		this.boardArray[choosenTile.row][choosenTile.col].tileSprite.alpha = 1;
    		this.tweens.add({
    			targets: [this.boardArray[choosenTile.row][choosenTile.col].tileSprite],
    			alpha: 1,
    ...
    isLegalPosition(row, col, value) {
    	...
    	if (!rowInside || !colInside) return false;
    	if (this.boardArray[row][col].tileValue == 12) return false;
